...
object ValueOps {
    ...
    def plus(v1: Value, v2: Value): Value = (v1,v2) match{
        ...
        case _ => throw new IllegalArgumentException("Cannot add figure to value")
    }
    ...
    def mult(v1: Value, v2:Value): Value = (v1,v2) match{
        ...
        case _=> throw new IllegalArgumentException("Cannot multiply Figure with value")

    }

    def div(v1: Value, v2:Value): Value = (v1,v2) match{
        case (NumValue(f1), NumValue(f2))=>{
            if(f2 != 0){ NumValue(f1/f2)}
